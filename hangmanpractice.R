# Set the file path to access the list of dictionary words in Doc3.txt
local_file_path <- "Doc3.txt"

# Dictionary-List will read the words in Doc2.txt
dictionary_list <- readLines("Doc3.txt")

# Choosing a random element from the list using the 
correct_word <- tolower(sample(dictionary_list, 1))

#providing the user with the number of letters in the word
#using the nchar function to provide the number of letters
#Using the cat function to concatenate and print a text message that includes the number of characters
word_length <- nchar(correct_word)
cat("The word has", word_length,"letters,")

#Defined the number of wrong guesses allowed, and concatenated it with text to be printed to the user in the console
# I have added a space infront of "you" since it will form a sentence with the print in line 14
wrong_guesses_allowed <- 7
cat("you have", wrong_guesses_allowed, "wrong guesses allowed)
...
cat(" you are allowed", wrong_guesses_allowed,"wrong guesses.\n")
